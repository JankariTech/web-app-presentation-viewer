Feature: markdown presentation viewer
  ...
  Scenario: preview markdown file using custom template
    ...
    And the content of the current slide should be "WEB APP PRESENTATION VIEWER WITH CUSTOM TEMPLATES"


  Scenario: preview markdown file using custom template with templates and markdown file inside a folder
    Given user "admin" creates a folder "templates-folder" using API
    ...
    And the content of the current slide should be "WEB APP PRESENTATION VIEWER WITH CUSTOM TEMPLATES WITH TEMPLATE AND MARKDOWN INSIDE SAME FOLDER"
